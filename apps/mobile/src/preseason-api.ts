...
type BlendStanding=Standing&{currentPlayed?:number;historicalPriorWeight?:number};
...
export function blendStanding(current?:Standing,previous?:Standing):BlendStanding|undefined{
 if(!previous)return current&&current.played>=3?{...current,currentPlayed:current.played,historicalPriorWeight:0}:undefined;
 if(!current||current.played<=0)return{...previous,currentPlayed:0,historicalPriorWeight:1};
