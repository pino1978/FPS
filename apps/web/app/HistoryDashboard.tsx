...
function SystemCard({item}:{item:any}){return <div className="history"><span><b>{item.mode} · {item.selections?.length??0} selezioni · {item.combinations?.length??0} combinazioni</b><small>{execution(item)}{item.bookmaker?` · ${item.bookmaker}`:''} · costo €{Number(item.totalCost??0).toFixed(2)} · {date(item.createdAt)}</small>{(item.selections||[]).slice(0,3).map((s:any)=><small key={s.id}>{s.selection} · {s.market}{s.originalPrediction?` · P ${pct(s.originalPrediction.probability)} · Conf ${pct(s.originalPrediction.confidence)} · ${s.originalPrediction.modelVersion}`:' · prediction originaria non disponibile'}</small>)}</span><strong className={`status status-${String(item.status).toLowerCase()}`}>{item.status}</strong></div>}
