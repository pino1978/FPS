...
export default function SystemBuilderPanel({bag,setBag,available}:{bag:SystemPick[];setBag:Dispatch<SetStateAction<SystemPick[]>>;available:SystemPick[]}){
 ...
 const [analysis,setAnalysis]=useState<any>(),[actualOdds,setActualOdds]=useState<Record<string,string>>({});
 ...
 return <div className="panel"><small>CREAZIONE GUIDATA</small><h2>System Builder</h2><p>Automatico seleziona opportunità valide; Assistito sviluppa i pronostici scelti; Manuale espone la combinatoria. Le incompatibilità sono sempre bloccate.</p>
  ...
  {analysis?.status==='INCOMPATIBLE'&&<div className="notice"><b>INCOMPATIBLE</b><span>{analysis.reason}</span></div>}
  {analysis?.status==='OK'&&analysis.correlations?.length>0&&<div className="notice"><b>Correlazioni rilevate</b><span>{analysis.correlations.length} relazione/i classificate; non sono incompatibilità e saranno considerate dall’ottimizzatore.</span></div>}
  <button className="primary" disabled={busy||!source.length||blocked} onClick={build}>{busy?'Elaborazione…':blocked?'Rimuovi le incompatibilità':'Genera sistema'}</button>
...
function SystemResult({result,budget,fallbackStake}:{result:any;budget:number;fallbackStake:number}){if(result.status!=='OK')return <div className="notice"><b>{result.status==='NO_BET'?'NO_BET':result.status}</b><span>{result.reason||'Il sistema non può essere generato con i vincoli correnti.'}</span></div>;const comboStake=Number(result.stake??fallbackStake);return <div className="systemSummary"><div><small>COMBINAZIONI</small><b>{result.combinations?.length??0}</b></div><div><small>STAKE / COMBO</small><b>€{comboStake.toFixed(2)}</b></div><div><small>COSTO</small><b>€{Number(result.cost||0).toFixed(2)}</b></div><div><small>BUDGET</small><b>€{Number(budget).toFixed(2)}</b></div><div><small>RISCHIO / CORR.</small><b>{result.profile||'Custom'} {result.maxCorrelation!=null?`· ${Math.round(result.maxCorrelation*100)}%`:''}</b></div>{result.coverage&&<p><strong>Copertura:</strong> {result.coverage.explanation}<br/><span>{result.coverage.guarantee}</span></p>}</div>}
